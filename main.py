...
# Actual symptoms dict and diseases list
...
def signup():
    if request.method == 'POST':
        ...
        mongo.db.users.insert_one({"username": username,"email":email, "password": hashed_password , "mobile":mobile})
...
def check_symptoms():
    ...
    symptoms = list(zip(df['Symptom'], df['weight']))
    return render_template('symptom_list.html', symptoms=symptoms)
...
@app.route('/diets')
def diets_page():
    diets_list = diets.to_dict('records')
